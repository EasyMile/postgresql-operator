...
func cleanupFunction() {
	...
	// Delete all CronJobs and Secrets generated by the backup controller in pgbNamespace.
	Expect(deleteAllCronJobsInNamespace(ctx, k8sClient, pgbNamespace)).ToNot(HaveOccurred())
	Expect(deleteAllSecretsInNamespace(ctx, k8sClient, pgbNamespace)).ToNot(HaveOccurred())
...
func deleteCronJob(ctx context.Context, cl client.Client, name, namespace string) error {
	cj := &batchv1.CronJob{}

	return deleteObject(ctx, cl, name, namespace, cj)
}

func deleteAllCronJobsInNamespace(ctx context.Context, cl client.Client, namespace string) error {
	...
	for i := range list.Items {
		if err := deleteCronJob(ctx, cl, list.Items[i].Name, namespace); err != nil {
			return err
		}
	}
...
func deleteAllSecretsInNamespace(ctx context.Context, cl client.Client, namespace string) error {
	...
	for i := range list.Items {
		if err := deleteSecret(ctx, cl, list.Items[i].Name, namespace); err != nil {
			return err
		}
	}
...
func hasEnvFromSecret(container corev1.Container, secretName string) bool {
...
func makePgDumpCronJobSpec() *batchv1.CronJobSpec {
	return &batchv1.CronJobSpec{
		Schedule: "*/5 * * * *",
		JobTemplate: batchv1.JobTemplateSpec{
			Spec: batchv1.JobSpec{
						...
						InitContainers: []corev1.Container{
							{
								Name:    "pg-dump",
								Image:   "postgres:16",
								Command: []string{"sh", "-c", "pg_dump --version"},
							},
						},
						Containers: []corev1.Container{
							{
								Name:    "finish",
								Image:   "ubuntu:24.04",
								Command: []string{"sh", "-c", "echo finished"},
							},
						},
					},
				},
...
// createPGBP creates a PostgresqlBackupProvider in pgbNamespace so that it
// shares a namespace with the backup object (required for cross-resource owner
// references to work).
func createPGBP(
	cronJobSpec *batchv1.CronJobSpec,
	labels, annotations map[string]string,
	prefix string,
) *postgresqlv1alpha1.PostgresqlBackupProvider {
...
// waitForBackupPhase creates a PostgresqlBackup and blocks until the operator
// moves it out of BackupNoPhase (either Valid or Error). The returned item is
// always freshly fetched.
func waitForBackupPhase(it *postgresqlv1alpha1.PostgresqlBackup) *postgresqlv1alpha1.PostgresqlBackup {
	...
	Eventually(
		func() error {
			...
			if it.Status.Phase == postgresqlv1alpha1.BackupNoPhase {
				return gerrors.New("pgb hasn't been updated by operator")
			}
